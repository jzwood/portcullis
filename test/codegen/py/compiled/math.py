...
def tail(xs):
  return (xs if int(xs == []) else identity2(xs[0])(xs[1:]))
...
def take(xs):
  return lambda n: ([] if int(xs == []) else _take(n)(xs[0])(xs[1:]))
...
def merge2(ys):
  return lambda x: lambda xs: ([x] + xs if int(ys == []) else merge3(x)(xs)(ys[0])(ys[1:]))

# signature: ([Num] -> ([Num] -> [Num]))
def merge(xs):
  return lambda ys: (ys if int(xs == []) else merge2(ys)(xs[0])(xs[1:]))
...
# signature: ([Num] -> Num)
def total(xs):
  return (0.0 if int(xs == []) else _total(xs[0])(xs[1:]))
...
def rankPet(p1):
  return lambda p2: ((p1, p2) if int(CHIPMUNK == p1) else (p2, p1))
...
# signature: ([Num] -> [Num])
def qsort(xs):
  return (xs if int(xs == []) else qsortp(xs[0])(xs[1:]))
